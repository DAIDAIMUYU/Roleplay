...
const navItems = [
  { to: "/", label: "酒馆大厅", icon: <Home className="h-5 w-5" /> },
...
export function DesktopSidebar() {
  ...
  return (
    <aside className="w-56 flex flex-col bg-white border-r border-surface-100 flex-shrink-0 h-full">
      ...
      <nav className="flex-1 px-2 space-y-0.5">
        {navItems.map(({ to, label, icon }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-app text-sm font-medium transition-colors ${
                isActive
      ...
      <div className="px-3 py-4 border-t border-surface-100">
        <div className="flex items-center justify-between">
          <span className="text-xs text-ink-300">
            {isGuestOrDemo ? "访客模式" : isOwner ? "站主" : isAdmin ? "管理员" : ""}
          </span>
          <ModeBadge />
        </div>
        {isGuestOrDemo && (
          <p className="mt-1.5 text-xs text-ink-300 leading-relaxed">
            访客 Demo 不调用真实 AI，不写正式数据库
          </p>
        )}
      </div>
    </aside>
