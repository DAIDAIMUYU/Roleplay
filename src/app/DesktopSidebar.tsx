...
export function DesktopSidebar() {
  ...
  return (
    <aside className={`flex h-full flex-shrink-0 flex-col border-r border-surface-100 bg-white transition-all duration-200 ${collapsed ? "w-16" : "w-56"}`}>
      <div className={`px-4 py-5 ${collapsed ? "flex justify-center" : ""}`}>
        {collapsed ? (
          <Link to="/" className="flex h-8 w-8 items-center justify-center rounded-lg bg-brand-500">
      ...
      <div className={`border-t border-surface-100 px-3 py-4 ${collapsed ? "flex justify-center" : ""}`}>
        {collapsed ? (
          <SidebarCollapseButton collapsed={collapsed} onToggle={toggleCollapsed} />
        ) : (
          <>
